  ...
  useEffect(() => {
    ...
    for (let i = 0; i < size; i++) {
      ...
      for (let j = 0; j < size; j++) {
        const patternY = Math.floor((i / size) * heartPattern.length)
        const patternX = Math.floor((j / size) * heartPattern[0].length)
        
        const isHeart = heartPattern[patternY] && heartPattern[patternY][patternX] === '*'
        
        if (isHeart) {
          const value = 200 + Math.random() * 40
          row.push(value)
        } else {
